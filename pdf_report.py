...
def _page_3_bar_overview(ranked, styles):
    ...
    story.append(_box("Einordnung der Prozentwerte", [
        Paragraph("<b>0–25 %:</b> niedrig ausgeprägt (hier entsteht typischer Leistungsverlust unter Druck)", styles["P"]),
        Paragraph("<b>25–75 %:</b> mittel ausgeprägt (flexibel – kann tragen oder kippen, je nach Kontext)", styles["P"]),
...
def _page_4_meaning_cards(top3, bottom2, styles):
    ...
    story.append(Paragraph("Hier siehst du glasklar, wie du unter Druck performst - wo du <b>Leistung gewinnst</b> und wo du sie unnötig verlierst.", styles["Muted"]))
    story.append(Spacer(1, 6))
    ...
    story.append(Paragraph("Merksatz: Top-Hebel sind deine stärksten Wirkungen. Reibungszonen sind die Stellen, wo du unnötig Leistung verlierst – beides ist steuerbar.", styles["Muted"]))
    return story
...
def _meaning_web_card(fid: str, pct: int, mode: str, styles):
    ...
    # Card-Title Style (größer wie im Web)
    ...
    t.setStyle(TableStyle([
        ...
        # Steuerung: oben Linie + etwas Luft, dann kompakt
