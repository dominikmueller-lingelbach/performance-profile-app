...
from pdf_report import build_pdf_report, MEANING_CARDS
from report_content import FUNCTION_NAMES, TYPE_MAP
...
async def show_result(request: Request, report_id: str):
    ...
    # ✅ Backend-Maps fürs Webreport (müssen exakt dem PDF entsprechen)
    payload["function_names"] = FUNCTION_NAMES
    payload["type_map"] = TYPE_MAP
    payload["meaning_cards"] = MEANING_CARDS

    # ✅ Falls alte Reports noch kein top3/bottom2 gespeichert haben: Fallback
    if "top3" not in payload or "bottom2" not in payload:
        ranked = payload.get("ranked") or []

        # ranked kann list oder dict sein -> normalize minimal
        if isinstance(ranked, dict):
            ranked = list(ranked.items())

        ranked = [(str(fid), int(round(float(pct)))) for fid, pct in ranked]
        ranked = sorted(ranked, key=lambda x: x[1], reverse=True)

        top3_pairs = ranked[:3]
        bottom2_pairs = list(reversed(ranked[-2:]))

        payload["top3"] = [
            {"fid": fid, "name": FUNCTION_NAMES.get(fid, fid), "percent": pct, "addendum": ""}
            for fid, pct in top3_pairs
        ]
        payload["bottom2"] = [
            {"fid": fid, "name": FUNCTION_NAMES.get(fid, fid), "percent": pct}
            for fid, pct in bottom2_pairs
        ]
    ...
    return templates.TemplateResponse(
        "results.html",
        {
            ...
            # optional, falls irgendwo direkt genutzt:
...
async def submit(request: Request):
    ...
    save_report(report_id, {
        ...
        # ===== Top- & Bottom-Hebel (fix für PDF + Web)
        "top3": [
            {
                "fid": it.fid,
                "name": it.name,
